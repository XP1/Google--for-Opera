...
(function ()
{
    ...
    var avatarLink = "//lh5.googleusercontent.com/-0000000000/AAAAAAAAAAI/AAAAAAAAAAA/0000000/000-0/photo.jpg"; // Initialized to example link only.
    ...
    function fixHtml()
    {
        ...
        var gbmpdvSpan = "<span id='gbmpid' style=\"display:none\"></span><img id='gbmpi' width='96' height='96' onerror=\"window.gbar&&gbar.ppe?gbar.ppe():this.loadError=1;\" src=\"" + avatarLink + "\" alt=\"\">";
        gbmpdvElement.firstChild.insertAdjacentHTML("beforeBegin", gbmpdvSpan); // Insert the span before "<div class=gbpc>".
    ...
    window.addEventListener("DOMContentLoaded", function ()
    {
        ...
        handleException(fixCss);
        handleException(fixHtml);
    }, false);

    window.opera.addEventListener("BeforeScript", function (event)
    {
        if (!isAlreadyReplaced && (event.element.text.length >= 8000 && event.element.text.length <= 8500) && event.element.text.indexOf("window.jstiming.load.tick('ogjl');") !== -1)
        {
