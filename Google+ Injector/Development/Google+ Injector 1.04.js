...
(function ()
{
    ...
    if (window.self === window.top) // Only run the script for and attach event listeners to the top-level window.
    {
         ...
         * Depending on the language set in Google+, the text must obviously match the language.
        ...
        var locale = (function ()
        {
            ...
            /* Don't need */
            var openingMessageRegex = /if\(a\)\{X\(a,[\r\n]"(.+?)"\);/g;
            var openingMessage = "Opening&hellip;"; // Default: "Opening&hellip;".
            var serviceUnavailableErrorRegex = /\{X\(a,"(.+?)","%1\$s"\);/g;
            var serviceUnavailableError = "This service is currently unavailable.%1$sPlease try again later."; // Default: "This service is currently unavailable.%1$sPlease try again later.".
            ...
            var publicMembers =
            {
                findNotificationText: function ()
                {
                    ...
                    {
                        notificationText = window.top.document.selectNodes("//a[@href=\"/notifications/all\" and @class=\"d-h a-b-h-Jb a-la-h a-la-hA\"]")[0].innerHTML; // Find notification text from the left sidebar link.
                    });
                },
                ...
                findShareText: function ()
                {
                    handleException(function ()
                    {
                        shareText = window.top.document.selectNodes("//span[@role=\"button\" and @class=\"d-h a-b-f-i-Zd-h\"]")[0].innerHTML; // Find share text from the share links on the posts of the stream.
                    });
                },
                ...
                findOpeningMessage: function (text)
                {
                    handleException(function ()
                    {
                        openingMessage = openingMessageRegex.exec(text)[1];
                    });
                },
                getOpeningMessage: function ()
                {
                    return openingMessage;
                },
                findServiceUnavailableError: function (text)
                {
                    handleException(function ()
                    {
                        language = serviceUnavailableErrorRegex.exec(text)[1];
                    });
                },
                getServiceUnavailableError: function ()
                {
         ...
         * Use the "find" functions to find the user data in the document first before calling the "get" functions to return the private variable.
         * If a "find" function fails to find the text in the document, then the default null value stored in the private variable will be returned.
         */
        var user = (function ()
        {
            ...
            var publicMembers =
            {
                ...
                {
                    if (arguments.length === 0) // Search document.
                    {
                        handleException(function ()
                        {
                            var profileLinkNumberRegex = /\/u\/(.+?)\//g;
                            number = profileLinkNumberRegex.exec(window.top.document.selectNodes("//a[@id=\"gbmplp\"]")[0].href)[1]; // Find the account's user number used for each account in a multilogin account.
                        });
                    }
                    else if (arguments.length === 1) // Search argument.
                    {
                        handleException(function ()
                        {
                            number = numberRegex.exec(text)[1];
                            window.opera.postError("[Google+ Injector] number: " + number + "."); // Uncomment
                        });
                    }
                    else
                    {
                        window.opera.postError("[Google+ Injector] user.findNumber(): Too many arguments: " + arguments.length + ".");
                    }
                },
                getNumber: function ()
                {
                ...
                findAvatarLink: function ()
                {
                    handleException(function ()
                    {
                        avatarLink = window.top.OZ_initData["2"][1][3] + "?sz=48"; // Find the avatar link from the data arrays and then limit avatar size to 48x48 pixels.
                    });
                },
        ...
        var injectHtml = function ()
        {
            ...
            var gbmpdvSpanHtml = "<span id='gbmpid' style=\"display:none\"></span><img id='gbmpi' width='96' height='96' onerror=\"window.gbar&&gbar.ppe?gbar.ppe():this.loadError=1;\" src=\"" + user.getAvatarLink() + "\" alt=\"\">";
        ...
        var injectJs = function (event)
        {
            ...
            {
                locale.findNotificationText();
                locale.findShareText();
                locale.findLanguage(event.element.text);

                user.findNumber(event.element.text);
                user.findData(event.element.text);
                ...
                var js = "(function(){try{var d=true,i=false;var l=window.gbar=window.gbar||{};function _tvn(a,b){var c=parseInt(a,10);return isNaN(c)?b:c}function _tvf(a,b){var c=parseFloat(a);return isNaN(c)?b:c}function _tvb(a,b){return a==\"true\"?d:a==\"false\"?i:b}function _tvv(a){return!!a}function n(a,b,c){(c||l)[a]=b}l.bv={n:_tvn(\"2\",0),r:\"\",m:_tvn(\"1\",1)};var aa=function(){return l.bv.m==1};n(\"sb\",aa);"
                    ...
                    + "function _mlToken(a,b){try{if(G<1){G++;var c,e=a,f=b||{},g=encodeURIComponent,h=[\"//www.google.com/gen_204?atyp=i&zx=\",(new Date).getTime(),\"&jexpid=\",g(\"31139\"),\"&srcpg=\",g(\"prop=119\"),\"&jsr=\",Math.round(1/F),\"&ogv=\",g(\"1311276124.1311272413\")];if(f._sn)f._sn=\"og.\"+f._sn;for(var m in f){h.push(\"&\");h.push(g(m));h.push(\"=\");h.push(g(f[m]))}h.push(\"&emsg=\");h.push(g(e.name+\":\"+e.message));var j=h.join(\"\");if(H(j))j=j.substr(0,2E3);c=j;var k=window.gbar.logger._aem(a,"
                    ...
                    + "l.addHover(a)}else f.appendChild(g)}catch(Ca){o(Ca)}},ta=function(a,b,c){sa(a,b,c)},ua=function(a,b){sa(a,\"gbe\",b)},va=function(){z(function(){l.pcm&&l.pcm()})},wa=function(a,b,c,e,f,g,h,m){z(function(){l.paa&&l.paa(a,b,c,e,f,g,h,m)})},xa=function(a,b){K[a]||(K[a]=[]);K[a].push(b)},ya=function(a,b){N[a]||(N[a]=[]);N[a].push(b)},za=function(a,b){O[a]||(O[a]=[]);O[a].push(b)},pa=function(a){a.preventDefault&&a.preventDefault();a.returnValue=i;a.cancelBubble=d},U=null,oa=function(a,b){R();if(a){V(a,"
                    + "\"Opening&hellip;\");W(a,d);var c=typeof b!=\"undefined\"?b:1E4;U=window.setTimeout(Aa,c)}},Ba=function(a){R();if(a){W(a,i);V(a,\"\")}},Aa=function(a){R();if(a=a||document.getElementById(P)){V(a,\"This service is currently unavailable.%1$sPlease try again later.\",\"%1$s\");W(a,d)}},V=function(a,b,c){if(a&&b){var e=na(a);if(e){if(c){e.innerHTML=\"\";b=b.split(c);c=0;for(var f;f=b[c];c++){var g=document.createElement(\"div\");g.innerHTML=f;e.appendChild(g)}}else e.innerHTML=b;W(a,d)}}},W=function(a,b){var c=b!==undefined?b:d;c?Q(a,\"gbmsgo\"):"
                    + "S(a,\"gbmsgo\")},na=function(a){for(var b=0,c;c=a.childNodes[b];b++)if(T(c,\"gbmsg\"))return c},R=function(){U&&window.clearTimeout(U)};n(\"so\",ka);n(\"sos\",J);n(\"si\",la);n(\"tg\",qa);n(\"close\",ra);n(\"addLink\",ta);n(\"addExtraLink\",ua);n(\"pcm\",va);n(\"paa\",wa);n(\"ddld\",oa);n(\"ddrd\",Ba);n(\"dderr\",Aa);n(\"ca\",Q);n(\"cr\",S);n(\"cc\",T);n(\"rtl\",ma);n(\"bh\",K);n(\"abh\",xa);n(\"dh\",N);n(\"adh\",ya);n(\"ch\",O);n(\"ach\",za);n(\"qs\",function(){});var Da={};u.base=Da;n(\"wg\",{rg:{}});"
                    + "var Ea={tiw:_tvn(\"15000\",0),tie:_tvn(\"30000\",0)};u.wg=Ea;var Fa={thi:_tvn(\"10000\",0),thp:_tvn(\"180000\",0),tho:_tvn(\"5000\",0)};u.wm=Fa;var Ga=0,Ia=function(a){if(a!=Ga){Ga=a;p(Ha.i,{c:a})}},Ja=function(a){var b=document.getElementById(\"gbmpp\");if(b)b.style.display=a?\"block\":\"none\"};w.push([\"m\",{url:\"//ssl.gstatic.com/gb/js/smm_a6293424e17d62c94c657d876c684fd6.js\"}]);"
                    + "n(\"logNotificationsCountUpdate\",Ia);n(\"depl\",Ja);var Ka={};u.heavy=Ka;if(_tvv(\"1\")){var La=_tvb(\"true\",i);w.push([\"gc\",{auto:La,url:\"https://ssl.gstatic.com/gb/js/gcm_d6fc0113471d6e272d0a97288bf902eb.js\"}]);var Ma={version:\"gcm_d6fc0113471d6e272d0a97288bf902eb.js\",index:\"0\",lang:\"" + locale.getLanguage() + "\"};u.gc=Ma;var X=function(a){if(window.googleapis)a&&a();else{a&&D(a);C(\"gc\")}};n(\"lGC\",X);_tvv(\"1\")&&n(\"lPWF\",X)}"
                    + "if(_tvv(\"1\")&&_tvv(\"1\")){var Y=function(a){X(function(){y(\"pw\",a);C(\"pw\")})};n(\"lPW\",Y);w.push([\"pw\",{url:\"//ssl.gstatic.com/gb/js/pwm_0e44f32cbac2196df4c324ca3ab2ecf7.js\"}]);var Na=[],Oa=function(a){Na[0]=a},Pa=function(a,b){var c=b||{};c._sn=\"pw\";o(a,c)},Qa={signed:Na,elog:Pa,base:\"https://plusone.google.com/u/" + user.getNumber() + "\",loadTime:(new Date).getTime()};u.pw=Qa;var Ra=function(a,b){for(var c=b.split(\".\"),e=function(){var m=arguments;a(function(){for(var j=l,k=0,v=c.length-1;k<v;++k)j=j[c[k]];j[c[k]].apply(j,m)})},f=l,g=0,h=c.length-"
                    + "1;g<h;++g)f=f[c[g]]=f[c[g]]||{};return f[c[g]]=e};Ra(Y,\"pw.clk\");Ra(Y,\"pw.hvr\");n(\"su\",Oa,l.pw)}var Sa=function(a){y(\"uf\",a)};w.push([\"uf\",{url:\"//ssl.gstatic.com/gb/js/fm_e6150f2ae3471582f5e836c0e2ecbb3e.js\"}]);var Z={};Z.j=function(a){l.feedback.ds=a};Z.k=function(){Sa(function(){l.feedback.startFeedback()});C(\"uf\")};n(\"feedback\",Z);n(\"startFeedback\",Z.k,Z);n(\"dsc\",Z.j,Z);var Ta={productId:\"sandbar:119\",locale:\"" + locale.getLanguage() + "\"};u.feedback=Ta;"
                    + "if(_tvv(\"\")){var $={},Ua=function(a,b){$[a]||($[a]=[]);$[a].push(b)},Va=function(a){if(a.type)for(var b=$[a.type],c=0;b&&c<b.length;++c)try{b[c](a)}catch(e){o(e)}};n(\"bc\",{subscribe:Ua,dispatch:Va})}var Wa={w:1,A:2,O:3,o:4,N:5,D:6,i:7,F:8,S:9,M:10,C:11,L:12,K:13,G:14,J:15,I:16,Q:17,v:18,H:19,R:20,P:21,u:22,z:23,U:24,V:25,T:26,a:27,B:500},Xa=_tvf(\"0.001\",1.0E-4),Ha=Wa;"
                    + "function Ya(a,b){var c=encodeURIComponent,e=[\"//www.google.com/gen_204?atyp=i&zx=\",(new Date).getTime(),\"&oge=\",a,\"&ogex=\",c(\"31139\"),\"&ogp=\",c(\"119\"),\"&ogsr=\",Math.round(1/Xa),\"&ogv=\",c(\"1311276124.1311272413\")];if(b){if(\"ogw\"in b){e.push(\"&ogw=\"+b.ogw);delete b.ogw}var f;f=b;var g=[],h;for(h in f){g.length!=0&&g.push(\",\");g.push(h);g.push(\".\");g.push(f[h])}f=g.join(\"\");if(f!=\"\"){e.push(\"&ogad=\");e.push(c(f))}}s(e.join(\"\"))}"
                    + "if(_tvv(\"1\")&&Math.random()<Xa){p=Ya;n(\"il\",p,t);var Za={};u.il=Za}if(_tvv(\"1\")||_tvv(\"1\")){var $a={uo:\"\",s:\"/\",h:\"/\",po:\"\",pa:\"/u/" + user.getNumber() + "/_/notifications/frame\",q:\"\",f:\"pid=119\",ipd:_tvn(\"5\",5),psec:_tvn(\"\",-1),it:_tvn(\"60\",-1),l:\"" + locale.getLanguage() + "\",sto:_tvn(\"10\",10),mnr:_tvn(\"30\",0)};u.sw=$a}"
                    + "if(_tvv(\"1\")){var ab=[],bb=function(a){ab.push(a)};n(\"nuc\",ab);n(\"anuc\",bb);var cb={ht:_tvn(\"100\",0),n:\"" + locale.getNotificationText() + "\",es:_tvn(\"0\",0)};u.no=cb}if(_tvv(\"1\")){var db=[],eb=function(a){db.push(a)};n(\"smc\",db);n(\"asmc\",eb);var fb={ht:_tvn(\"100\",0),n:\"" + locale.getShareText() + "\"};u.sb=fb}var gb=function(){l.prm&&l.prm()},hb=function(a){y(\"m\",function(){l.spn(a)})},ib=function(a){y(\"m\",function(){l.spp(a)})};"
                    + "n(\"spn\",hb);n(\"spp\",ib);xa(\"gbd4\",gb);if(_tvb(\"true\",d)){var jb={" + user.getData() + "};u.prf=jb}"
                    + "function kb(){function a(){if(document.getElementById(\"gb\")==null)p(Ha.a,{_m:\"nogb\"});else{for(var k;k=g[h++];)if(k[0]==\"m\"||k[1].auto)break;if(k){B(2,k[0]);A(k[1].url)}h<g.length&&setTimeout(a,0)}}function b(){f-- >0?setTimeout(b,0):a()}var c=_tvb(\"true\",d),e=_tvb(\"false\",i),f=3,g=w,h=0,m=window.gbarOnReady;if(m)try{m()}catch(j){o(j)}if(e)n(\"ldb\",a);else if(c)window.addEventListener?window.addEventListener(\"load\",b,i):window.attachEvent(\"onload\","
                    + "b);else b()}n(\"rdl\",kb);}catch(e){window.gbar&&gbar.logger&&gbar.logger.ml(e);}})();"
                ...
                js = "(function(){try{var d=true,i=false;var l=window.gbar=window.gbar||{};function _tvn(a,b){var c=parseInt(a,10);return isNaN(c)?b:c}function _tvf(a,b){var c=parseFloat(a);return isNaN(c)?b:c}function _tvb(a,b){return a==\"true\"?d:a==\"false\"?i:b}function _tvv(a){return!!a}function n(a,b,c){(c||l)[a]=b}l.bv={n:_tvn(\"2\",0),r:\"\",m:_tvn(\"1\",1)};var aa=function(){return l.bv.m==1};n(\"sb\",aa);"
                    ...
                    + "n(\"logNotificationsCountUpdate\",Ha);n(\"depl\",Ia);var Ja={};u.heavy=Ja;if(_tvv(\"1\")){var Ka=_tvb(\"true\",i);w.push([\"gc\",{auto:Ka,url:\"https://ssl.gstatic.com/gb/js/gcm_b1be572aff2630578d6077ebe3f660a9.js\"}]);var La={version:\"gcm_b1be572aff2630578d6077ebe3f660a9.js\",index:\"0\",lang:\"" + locale.getLanguage() + "\"};u.gc=La;var X=function(a){if(window.googleapis)a&&a();else{a&&D(a);C(\"gc\")}};n(\"lGC\",X);_tvv(\"1\")&&n(\"lPWF\",X)}"
                    + "if(_tvv(\"1\")&&_tvv(\"1\")){var Y=function(a){X(function(){y(\"pw\",a);C(\"pw\")})};n(\"lPW\",Y);w.push([\"pw\",{url:\"//ssl.gstatic.com/gb/js/pwm_377e54339fe6909b01c5d023b5297ebb.js\"}]);var Ma=[],Na=function(a){Ma[0]=a},Oa=function(a,b){var c=b||{};c._sn=\"pw\";o(a,c)},Pa={signed:Ma,elog:Oa,base:\"https://plusone.google.com/u/" + user.getNumber() + "\",loadTime:(new Date).getTime()};u.pw=Pa;var Qa=function(a,b){for(var c=b.split(\".\"),e=function(){var m=arguments;a(function(){for(var j=l,k=0,v=c.length-1;k<v;++k)j=j[c[k]];j[c[k]].apply(j,m)})},f=l,g=0,h=c.length-"
                    + "1;g<h;++g)f=f[c[g]]=f[c[g]]||{};return f[c[g]]=e};Qa(Y,\"pw.clk\");Qa(Y,\"pw.hvr\");n(\"su\",Na,l.pw)}var Ra=function(a){y(\"uf\",a)};w.push([\"uf\",{url:\"//ssl.gstatic.com/gb/js/fm_e6150f2ae3471582f5e836c0e2ecbb3e.js\"}]);var Z={};Z.a=function(a){l.feedback.ds=a};Z.i=function(){Ra(function(){l.feedback.startFeedback()});C(\"uf\")};n(\"feedback\",Z);n(\"startFeedback\",Z.i,Z);n(\"dsc\",Z.a,Z);var Sa={productId:\"sandbar:119\",locale:\"" + locale.getLanguage() + "\"};u.feedback=Sa;"
                    ...
                    + "if(_tvv(\"1\")&&Math.random()<Va){p=Wa;n(\"il\",p,t);var Xa={};u.il=Xa}if(_tvv(\"1\")||_tvv(\"1\")){var Ya={uo:\"\",s:\"/\",h:\"/\",po:\"\",pa:\"/u/" + user.getNumber() + "/_/notifications/frame\",q:\"\",f:\"pid=119\",ipd:_tvn(\"5\",5),psec:_tvn(\"\",-1),it:_tvn(\"60\",-1),l:\"" + locale.getLanguage() + "\",sto:_tvn(\"10\",10),mnr:_tvn(\"30\",0)};u.sw=Ya}"
                    + "if(_tvv(\"1\")){var Za=[],$a=function(a){Za.push(a)};n(\"nuc\",Za);n(\"anuc\",$a);var ab={ht:_tvn(\"100\",0),n:\"" + locale.getNotificationText() + "\"};u.no=ab}if(_tvv(\"1\")){var bb=[],cb=function(a){bb.push(a)};n(\"smc\",bb);n(\"asmc\",cb);var db={ht:_tvn(\"100\",0),n:\"" + locale.getShareText() + "\"};u.sb=db}var eb=function(){l.prm&&l.prm()},fb=function(a){y(\"m\",function(){l.spn(a)})},gb=function(a){y(\"m\",function(){l.spp(a)})};n(\"spn\",fb);n(\"spp\",gb);"
                    + "xa(\"gbd4\",eb);if(_tvb(\"true\",d)){var hb={" + user.getData() + "};u.prf=hb}"
                ...
                window.opera.removeEventListener("BeforeScript", injectJs, false); // Remove event listener after JavaScript injection.
                window.opera.postError("[Google+ Injector] injectJs(): Injected JavaScript and then removed event listener.");
            }
        };

        var injectCssAndHtml = function ()
        {
            locale.findNotificationText();
            locale.findShareText();
            ...
            window.removeEventListener("DOMContentLoaded", injectCssAndHtml, false); // Remove event listener after CSS and HTML injection.
            window.opera.postError("[Google+ Injector] injectCssAndHtml(): Injected CSS and HTML and then removed event listener.");
        };

        window.top.addEventListener("DOMContentLoaded", injectCssAndHtml, false);
